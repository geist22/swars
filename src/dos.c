...
static bool find_file_in_directory (const char *directory, const char *filename,
...
static bool lookup_path(const char *path, char *buffer, size_t size)
{
      ...
      || strcmp(segment, FS_SEP_STR) == 0) /* absolute path */
    {
        ...
        if (strcmp(segment, FS_SEP_STR) == 0)
            len = snprintf(prefix, sizeof(prefix), "%s", FS_SEP_STR);
        else
            len = snprintf(prefix, sizeof(prefix), "%s%s", segment, FS_SEP_STR);
        if ((len < 1) || (len > (int)sizeof(prefix)))
            return false;

        rest = extract_path_segment(rest, segment, sizeof(segment));

        if (!find_file_in_directory(prefix, segment,
          exsegment, sizeof(exsegment)))
        {
    ...
    {
        if (!find_file_in_directory(".", segment,
          exsegment, sizeof(exsegment)))
        {
            snprintf(buffer, size, "%s", path);
            strtolower(buffer);
